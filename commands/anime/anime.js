...
module.exports = {
  ...
  callback({ message, args }) {
    ...
    if (!args[0]) {
      return message.channel.send("❓ | Please tell me what anime to search");
    }
    ...
    mal.search("anime", args, "page").then((info) => {
      const embed = new Discord.MessageEmbed()
        ...
        .addFields(
          {
            name: "📅 Release Date",
            value: `${info.results[0].start_date || "Not yet aired"}`,
            inline: true,
          },
          {
            name: "🗓️ End Date",
            value: `${
              info.results[0].end_date || "Unknown" || "Not yet aired"
            }`,
            inline: true,
          },
          ...
          {
            name: "💬 Rating",
            value: `${info.results[0].rated || "None"}`,
            inline: true,
          },
          {
            name: "🔩 Episodes",
            value: `${info.results[0].episodes || "Unknown"}`,
            inline: true,
          }
        )
