...
module.exports = {
  ...
  callback({ message, client, args }) {
    ...
    if (message.member.roles.highest.position < member.roles.highest.position) {
      return message.channel.send(
        `❌ | You cannot remove role to user who have higher role than you...`
      );
    }
    ...
    if (
      ...
      return message.channel.send(
        `❌ | The roles cannot be removed because they are higher than or equal to my highest role`
      );
    }
    ...
    message.channel.send(
      `:gear: | Role ${role} has been removed from ${member}`
    );
  },
};
